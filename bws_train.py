...
from src import BertDataset, BertProcessor, BertRegressor, BertRegTrainer 
...
def main(args):
    ...
    config = BertConfig.from_pretrained(os.path.join(training_config.base_model, 'bert-tiny', 'bert_config.json'))
    model = BertModel.from_pretrained(os.path.join(training_config.base_model, 'bert-tiny'), config=config)
    config.max_position_embeddings = 128
    
    # process data 
    X_train = pd.read_csv(os.path.join(training_config.data_path, 'bws_score_train.csv'))
    X_val = pd.read_csv(os.path.join(training_config.data_path, 'bws_score_val.csv'))
    ...
    # train model 
    model_reg = BertRegressor(config, model).to(training_config.device)    # add regression layer 
    bert_trainer = BertRegTrainer(config, training_config, model_reg, train_dataloader, val_dataloader)
